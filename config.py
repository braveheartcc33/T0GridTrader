"""
Grid Trader 配置
A股网格交易系统 - 配置参数
"""
import os

# ==================== 多股票配置列表 ====================
# 支持多只股票同时运行网格策略
STOCKS = [
    {
        'code': '159567.SZ',
        'name': '港股创新药',
        'base_shares': 100000,     # 初始底仓股数
        'shares_per_grid': 10000,   # 每格交易量
        'grid_count': 10,          # 网格档位
        'atr_spacing': 4.0,        # ATR倍数
        'stop_loss_enabled': False,
    },
    # 可以添加更多股票:
    # {
    #     'code': '512880.SZ',
    #     'name': '证券ETF',
    #     'base_shares': 50000,
    #     'shares_per_grid': 5000,
    #     'grid_count': 10,
    #     'atr_spacing': 3.5,
    #     'stop_loss_enabled': True,
    # },
]

# ==================== 向后兼容：单股票默认配置 ====================
# 如果 STOCKS 为空或只包含一只股票，使用这些默认配置
STOCK_CODE = STOCKS[0]['code'] if STOCKS else "159567.SZ"
STOCK_NAME = STOCKS[0]['name'] if STOCKS else "港股创新药"

# ==================== 网格参数 ====================
GRID_COUNT = STOCKS[0]['grid_count'] if STOCKS else 10           # 网格档位数量
STOP_LOSS_ENABLED = STOCKS[0]['stop_loss_enabled'] if STOCKS else False  # 是否启用止损
STOP_LOSS_PCT = -0.02     # 止损幅度 -2%（止损启用时生效）
INITIAL_BASE_SHARES = STOCKS[0]['base_shares'] if STOCKS else 100000  # 初始底仓股数

# 每格股数（固定，根据资金管理计算）
SHARES_PER_GRID = STOCKS[0]['shares_per_grid'] if STOCKS else 10000

# 基准价：开盘价（或昨日收盘价），系统启动时自动设置
BASE_PRICE = None         # 运行时设置

# ==================== 动态网格间距参数 ====================
# 2026-04-02 改用历史波动率（涨跌幅百分比），全天固定不变
# 每格基础间距 = 历史波动率 × 基准价 / GRID_COUNT
# 不再使用 ATR（无法处理除权）和分时段切换
HIST_VOL_PERIOD = 20  # 历史波动率计算周期（天数）
USE_HIST_VOL = True              # 开关：True=历史波动率，False=ATR

# ==================== 向后兼容的别名（已废弃） ====================
GRID_SPACING_RULES = []  # 已废弃
BASE_MULTIPLIER = 1.0    # 已废弃

# ==================== 向后兼容的别名（废弃） ====================
OPENING_MULTIPLIER = 1.75
CLOSING_MULTIPLIER = 0.70
MORNING_CLOSE_MULTIPLIER = 0.70
AFTERNOON_OPEN_MULTIPLIER = 1.75
OPENING_WINDOW_MIN = 30
CLOSING_WINDOW_MIN = 30

# ==================== 指标参数 ====================
ATR_PERIOD = 14           # ATR周期
BOLL_PERIOD = 20           # 布林带周期
BOLL_STD_MULT = 2          # 布林带标准差倍数

# ==================== 数据源 ====================
TUSHARE_TOKEN = os.getenv("TUSHARE_TOKEN") or "d11513bc2e258334d01ddf0db02d45793325443dc1260931691d1552"

# 腾讯实时行情接口
TENGXUN_REALTIME_URL = "https://qt.gtimg.cn/q="

# ==================== 飞书通知配置 ====================
# 可以为每只股票配置不同的webhook，或使用统一的webhook加后缀区分
FEISHU_WEBHOOK_URL = "https://open.feishu.cn/open-apis/bot/v2/hook/b001cf93-2f59-4289-8240-567c276e4144"
FEISHU_SIGNING_KEY = "Bwo5lC6e8X1EnNP3eBPaDh"

# ==================== 交易时间段（北京时间） ====================
TRADING_MORNING_START = (9, 30)
TRADING_MORNING_END = (11, 30)
TRADING_AFTERNOON_START = (13, 0)
TRADING_AFTERNOON_END = (15, 0)

# ==================== 轮询配置 ====================
POLL_INTERVAL_SEC = 10    # 实时行情轮询间隔（秒）

# ==================== 日志配置 ====================
LOG_LEVEL = "INFO"
LOG_FILE = "grid_trader.log"

# ==================== 多股票运行时配置 ====================
# 是否启用多股票模式
MULTI_STOCK_MODE = len(STOCKS) > 1

# 每只股票状态文件前缀
STATE_FILE_PREFIX = "state_"

# 每只股票独立运行在独立线程
ENABLE_MULTI_THREAD = True
